...
def run_subgraph_cover(
    *,
    ...
    if cap_methods:
        ...
        for w in wanted:
            if w in name_map:
                cap_methods_set.add(name_map[w])
            else:
                cap_methods_set.add(w)
    ...
    merged_cap = merged
    raw_cap = raw_subgraphs

    cap_ratio = float(max_view_report_ratio) if "max_view_report_ratio" in locals() else 1.0
    if cap_ratio < 1.0 and universe_reports:
        cap = int(len(universe_reports) * cap_ratio)
        merged_cap = [sg for sg in merged if len(sg.covered_report_ids) <= cap]
        raw_cap = [sg for sg in raw_subgraphs if len(sg.covered_report_ids) <= cap]
    ...
    B_cost_ref = float(sum(float(getattr(sg, "cost", 0.0)) for sg in cover.selected[: int(budget)]))
    if B_cost_ref <= 0:
        B_cost_ref = None
    ...
    ecfa_metrics = compute_metrics(
        cover, total_reports, len(universe), int(budget), universe_reports_set=universe_reports, budget_cost_ref=B_cost_ref
    )
    baselines.append(BaselineResult(name="ECFA", cover_results=[cover], metrics=ecfa_metrics))


    wo_merge_metrics = compute_metrics(
        cover_wo_merge, total_reports, len(universe), int(budget), universe_reports_set=universe_reports, budget_cost_ref=B_cost_ref
    )
    baselines.append(BaselineResult(name="ECFA (No Merge)", cover_results=[cover_wo_merge], metrics=wo_merge_metrics))


    random_runs: List[CoverResult] = []
    pool_random = _pool("Random-k", "merged")
    for i in range(30):
        random_runs.append(random_selection(pool_random, universe=universe, k=int(budget), seed=int(i)))

    random_metrics_list = [
        compute_metrics(
            r, total_reports, len(universe), int(budget), universe_reports_set=universe_reports, budget_cost_ref=B_cost_ref
        )
    ...
    curves_rho = np.array([m.get("rho_curve", [0.0]*int(budget)) for m in random_metrics_list], dtype=float)
    curves_cum = np.array([m.get("cum_cost_curve", [0.0]*int(budget)) for m in random_metrics_list], dtype=float)
    ...
    std_rho_curve = curves_rho.std(axis=0, ddof=0).tolist()
    avg_cum_curve = curves_cum.mean(axis=0).tolist()
    std_cum_curve = curves_cum.std(axis=0, ddof=0).tolist()
    ...
    auc_r_list = [m.get("sum@k_report", 0.0) for m in random_metrics_list]
    auc_e_list = [m.get("sum@k_entity", 0.0) for m in random_metrics_list]
    fin_r_list = [m.get("final_report_coverage", 0.0) for m in random_metrics_list]
    fin_e_list = [m.get("final_entity_coverage", 0.0) for m in random_metrics_list]

    naucB_r_list = [m.get("nAUC@B_report", 0.0) for m in random_metrics_list]
    naucB_e_list = [m.get("nAUC@B_entity", 0.0) for m in random_metrics_list]

    cov025_r_list = [m.get("Cov@0.25B_report", 0.0) for m in random_metrics_list]
    cov050_r_list = [m.get("Cov@0.50B_report", 0.0) for m in random_metrics_list]
    cov075_r_list = [m.get("Cov@0.75B_report", 0.0) for m in random_metrics_list]
    cov025_e_list = [m.get("Cov@0.25B_entity", 0.0) for m in random_metrics_list]
    cov050_e_list = [m.get("Cov@0.50B_entity", 0.0) for m in random_metrics_list]
    cov075_e_list = [m.get("Cov@0.75B_entity", 0.0) for m in random_metrics_list]
    ...
    avg_random_metrics = {
        ...
        "rho_curve_std": std_rho_curve,
        "cum_cost_curve_std": std_cum_curve,


        "sum@k_report": calculate_nauc(avg_report_curve, int(budget)),
        "sum@k_entity": calculate_nauc(avg_entity_curve, int(budget)),
        ...
        "std_nAUC@B_report": _std(naucB_r_list),
        "std_nAUC@B_entity": _std(naucB_e_list),
        "std_Cov@0.25B_report": _std(cov025_r_list),
        "std_Cov@0.50B_report": _std(cov050_r_list),
        "std_Cov@0.75B_report": _std(cov075_r_list),
        "std_Cov@0.25B_entity": _std(cov025_e_list),
        "std_Cov@0.50B_entity": _std(cov050_e_list),
        "std_Cov@0.75B_entity": _std(cov075_e_list),
    }
    ...
    top_uni_size_res = top_k_selection(pool_top_uni, universe=universe, k=int(budget), score_fn=lambda sg: len(sg.covered_universe))
    baselines.append(
        BaselineResult(
            name="TopUniSize-k",
            cover_results=[top_uni_size_res],
            metrics=compute_metrics(
                top_uni_size_res, total_reports, len(universe), int(budget), universe_reports_set=universe_reports, budget_cost_ref=B_cost_ref
            ),
        )
    )
    ...
    top_raw_size_res = top_k_selection(pool_top_raw, universe=universe, k=int(budget), score_fn=lambda sg: (len(sg.node_ids) + len(sg.edge_ids)))
    baselines.append(
        BaselineResult(
            name="TopRawSize-k",
            cover_results=[top_raw_size_res],
            metrics=compute_metrics(
                top_raw_size_res, total_reports, len(universe), int(budget), universe_reports_set=universe_reports, budget_cost_ref=B_cost_ref
            ),
        )
    )
...
def extract_feasible_subset_from_order(
    ...
    *,
    ...
    cum = 0.0
    for sg in ordered:
        if k_limit is not None and len(out) >= int(k_limit):
            break
...
def build_excel_sheets(
    *,
    ...
    cost_rows = []
    for b in baselines:
        rhos = list(b.metrics.get("rho_curve", []))
        cumc = list(b.metrics.get("cum_cost_curve", []))
        covr = list(b.metrics.get("report_coverage_curve", []))
        cove = list(b.metrics.get("entity_coverage_curve", []))
        k_len = max(len(rhos), len(cumc), len(covr), len(cove))
        if k_len <= 0:
            continue
        covr_std = list(b.metrics.get("report_coverage_std", [])) if b.name == "Random-k" else []
        cove_std = list(b.metrics.get("entity_coverage_std", [])) if b.name == "Random-k" else []
        for i in range(k_len):
            ...
            if b.name == "Random-k":
                row["cov_report_std"] = float(covr_std[i]) if i < len(covr_std) else float(covr_std[-1] if covr_std else 0.0)
                row["cov_entity_std"] = float(cove_std[i]) if i < len(cove_std) else float(cove_std[-1] if cove_std else 0.0)
    ...
    for app in apps:
        ...
        try:
            ...
            if not args.unit_cost:
                ...
                if args.cap_methods:
                    ...
                    for w in wanted:
                        if w in name_map:
                            ...
                            cap_methods_set.add(w)
                    if "ECFA w/o cost" in cap_methods_set:
                        use_cap = True

                pool_wo_cost = res.merged_subgraphs
                if use_cap:
                    cap_ratio = float(args.max_view_report_ratio)
                    if cap_ratio < 1.0 and res.universe_reports:
                        cap = int(len(res.universe_reports) * cap_ratio)
                        pool_wo_cost = [sg for sg in res.merged_subgraphs if len(sg.covered_report_ids) <= cap]
                ...
                B_cost_ref = float(sum(float(getattr(sg, "cost", 0.0)) for sg in res.cover.selected[: int(args.budget)]))
                if B_cost_ref <= 0:
                    B_cost_ref = None
