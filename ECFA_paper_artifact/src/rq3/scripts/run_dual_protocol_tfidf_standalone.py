...
def main() -> None:
    ap = argparse.ArgumentParser(description='Standalone TFIDF dual protocol runner')
    ...
    ap.add_argument('--global_topn', type=int, default=120)
    ap.add_argument('--cluster_topn', type=int, default=50)
    ap.add_argument('--ecfa_topn', type=int, default=80)
    args = ap.parse_args()
    ...
    clusters = cluster_tfidf_hc(corpus, TfidfBoundaryConfig(k=int(args.k), analyzer='char', ngram_min=2, ngram_max=4))
    write_csv(clusters, out_dir / f'clusters_tfidf_K{int(args.k)}.csv')
    ...
    rankings['ecfa_hybrid_allrel'] = build_hybrid_allrel_rankings(
        ...
        cfg=HybridAllRelConfig(
            global_topn=int(args.global_topn),
            cluster_topn=int(args.cluster_topn),
            ecfa_topn=int(args.ecfa_topn),
    ...
    summary_all, perq_metrics_all, byapp_all = evaluate_rankings_from_relmap(
        rankings_by_method={m: rankings[m] for m in methods},
        rel_map=allrel_map,
        boundary='tfidf_hc_K20',
        eval_keys=q_keys,
        success_ks=(10, 20),
        recall_ks=(20, 50),
        mrr_k=20,
    ...
    merged = summary_all.merge(summary_cross, on=['boundary', 'method'], suffixes=('_all', '_cross'))
    rows = []
    for _, r in merged.iterrows():
        row = {'boundary': r['boundary'], 'method': r['method']}
        for metric in ['success_at_20', 'mrr_at_20', 'recall_at_20', 'recall_at_50', 'ndcg_at_20']:
            a = float(r[f'{metric}_all'])
            c = float(r[f'{metric}_cross'])
            row[f'{metric}_all'] = a
            row[f'{metric}_cross'] = c
            row[f'{metric}_retention'] = (c / a) if a > 1e-12 else 0.0
        rows.append(row)
