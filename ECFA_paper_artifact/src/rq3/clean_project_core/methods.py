...
def _strict_rank_from_rows(app_ids: np.ndarray, q_scores: np.ndarray, rows: np.ndarray, qid: str, max_rank_cap: int) -> list[str]:


    if rows.size == 0:
        return []
    rows = np.asarray(rows, dtype=np.int32)
    local_scores = q_scores[rows]
    order = rows[np.argsort(-local_scores, kind='mergesort')]
    return dedup_keep_order(app_ids[order].tolist(), drop=qid, limit=max_rank_cap)
...
def rank_text_baselines(
    *,
    ...
    for app, qids in by_app_queries.items():
        ...
        scores, qids_valid, _ = idx.query_scores(qids)
        if len(qids_valid) == 0:
            continue
        ...
        for row_idx, qid in enumerate(qids_valid):
            ...
            rankings_by_method['global'][(app, qid)] = dedup_keep_order(
                app_ids[order_global].tolist(), drop=qid, limit=max_rank_cap
            )
            ...
            rankings_by_method['cluster_only'][(app, qid)] = _strict_rank_from_rows(
                app_ids=app_ids,
                q_scores=q_scores,
                rows=rows,
                qid=qid,
            ...
            for L in multi_Ls:
                ...
                rankings_by_method[f'multi_L{int(L)}'][(app, qid)] = _strict_rank_from_rows(
                    app_ids=app_ids,
                    q_scores=q_scores,
                    rows=rows,
                    qid=qid,
